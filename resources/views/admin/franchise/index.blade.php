...
<div class="row text-dark">
    ...
    <div class="col-12 mb-4">
        <div class="card bg-white border shadow-sm p-4">
            <div class="d-flex justify-content-between align-items-center flex-wrap">
                <div>
                    <h4 class="card-title text-dark font-weight-bold mb-1">Franchise Partnership Enquiries</h4>
                    <p class="card-description text-muted mb-0">Manage and track expansion requests, log budgets, assign staff, and track scheduling timelines.</p>
                </div>
                <div>
                    @if(hasPermission('franchise.export'))
                    <a href="{{ route('franchise.export', request()->all()) }}" class="btn btn-success px-4 mr-2">
                        <i class="mdi mdi-export mr-1"></i> Export list (CSV)
                    </a>
                    @endif
                    @if(hasPermission('franchise.edit'))
                    <a href="{{ route('franchise.create') }}" class="btn btn-primary px-4">
    ...
    <div class="col-12 mb-4">
        <div class="card bg-white border shadow-sm p-4">
            <h5 class="text-dark font-weight-bold mb-3 border-bottom pb-2">Filter Franchise requests</h5>
            <form action="{{ route('franchise.index') }}" method="GET" class="row">
                <!-- Search Input -->
                <div class="col-md-3 form-group">
                    <label class="text-dark font-weight-bold">Search Query</label>
                    <input type="text" name="search" class="form-control bg-white text-dark" placeholder="Name, Phone, City, State..." value="{{ request('search') }}">
                </div>
                ...
                <div class="col-12 mt-2 d-flex justify-content-end">
                    <a href="{{ route('franchise.index') }}" class="btn btn-secondary px-4 mr-2">Clear Filters</a>
                ...
                <table class="table table-bordered table-striped text-dark">
                    ...
                    <tbody>
                        @forelse($enquiries as $enq)
                            <tr>
                                <td class="font-weight-bold text-dark">{{ $enq->full_name }}</td>
                                <td>
                                    <div>Mobile: {{ $enq->mobile }}</div>
                                    <small class="text-muted">{{ $enq->email }}</small>
                                </td>
                                <td>{{ $enq->city }}, {{ $enq->state }}</td>
                                <td class="font-weight-bold text-success">{{ $enq->investment_budget }}</td>
                                <td>{{ $enq->current_business ?? 'None' }}</td>
                                <td>
                                ...
                                <td>
                                    <a href="{{ route('franchise.show', $enq->id) }}" class="btn btn-sm btn-info px-3">
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="text-center py-4 text-muted">
                                    <i class="mdi mdi-alert mr-1"></i> No franchise partnership requests found.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
